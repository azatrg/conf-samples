...
# Внешний интерфейс
export WAN=ens160
export WAN_IP=192.168.35.14

# Локальная сеть
export LAN1=ens192
export LAN1_IP_RANGE=192.168.10.0/23
...
#WРазрешаем подклчения к разным сервисам
#$IPT -A FORWARD -p tcp -i $LAN1 -o $WAN --dport 22 -j ACCEPT
#$IPT -A FORWARD -p tcp -i $LAN1 -o $WAN -s 192.168.11.45 -j ACCEPT
#$IPT -A FORWARD -p tcp -i $LAN1 -o $WAN --dport 80 -j ACCEPT
#$IPT -A FORWARD -p tcp -i $LAN1 -o $WAN --dport 443 -j ACCEPT

# Запретим доступ из локалки наружу для того что не разрешено
...
#Перенаправление обращений к сайтам на прокси-сервер squid
#$IPT -t nat -A PREROUTING -s 192.168.10.0/23 -p tcp -m multiport --dport 80,443 -j REDIRECT --to-port 3128
#$IPT -t nat -A PREROUTING -i $LAN1 -p tcp --dport 443 -j REDIRECT --to-port 3128
